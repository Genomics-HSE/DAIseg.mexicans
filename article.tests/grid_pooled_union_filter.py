"""
Post-inference grid search for downstream filtering, restricted to the pooled_union regime.

The script:
1) reads already computed predictions.pooled_union.tsv and ground truth files
2) rebuilds per-window modern callability (L_mod) from 1000G strict mask minus gaps
3) rebuilds per-window archaic observability as union_eff:
       |window ∩ strict ∩ (Vindija ∪ Altai ∪ Chagyr)| / |window ∩ strict|
4) evaluates a 2D grid of filters:
       L_mod > t_mod  and  union_eff >= t_union
5) recomputes precision / recall / F1 for ND_EU and ND_NA
6) saves a summary table
